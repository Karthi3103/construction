...
<form action="Show_Supervisor_Resource.jsp" method="post"> 
 <div class="row">
    <div class="col">
            <!-- Site Id input -->
        ...
        <label class="form-label " for="form2Example1" >Site Id</label>
           <select   class="form-select "  name="site_id"   aria-label="Default select example"  id="selectColor" required>
             ...
             <% try
             {
            	 ...
            	 ArrayList<String> customer_Site_id=JDBC.Get_Site_id(Supervisor_id);
             
        		
        		for(int j=0;j<customer_Site_id.size();j++)
        		{
        		
        	 %>
             <option  value="<%= customer_Site_id.get(j)%>"><%= customer_Site_id.get(j)%></option>
    ...
    <!-- Submit button -->
        ...
        <input type="submit" value="View" class=" form-control btn btn-secondary btn-block " id="form2Example1">
...
<th style="width:5%"><b>Entry Date</b></th>
<th style="width:5%"><b>Entry Time</b></th>
<th style="width:8%"><b>Male</b></th>
<th style="width:8%"><b>Female</b></th>
...
String query = "SELECT Site_id,date_format(Attendence_entry_date,'%m-%d-%Y'),time_format(Attendence_entry_time,'%h:%i:%p'),Male,Female from site_resource where Attendence_entry_date BETWEEN ? and ? and Site_id=?";
